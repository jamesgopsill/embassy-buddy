...
use crate::components::{
    adc::BuddyAdc, bed_power_monitor, buzzer, display, eeprom, ethernet::init_ethernet, fans::Fan,
...
pub use embassy_tmc::tmc2209::{
    ChopConf, GStat, Gconf, IHoldIRun, IfCnt, Ioin, NodeConf, PwmConf, TCoolThrs, TPowerDown,
    TStep, TpwmThrs, VActual,
};
...
impl<'a> BoardBuilder<'a> {
    ...
    fn build_bed_power_monitor(pin: PA3, adc: &'static BuddyAdc<ADC1>) -> BuddyBedPowerMonitor {
        bed_power_monitor::init_buddy_bed_power_monitor(adc, pin.degrade_adc())
    }
    ...
    pub async fn build(self) -> Board<'a> {
        ...
        if self.bed_heater || self.hotend_heater {
            let mut bed_pin = None;
            if self.bed_heater {
                bed_pin = Some(PwmPin::new_ch3(p.PB0, OutputType::PushPull));
            };
            let mut hotend_pin = None;
            if self.bed_heater {
                hotend_pin = Some(PwmPin::new_ch4(p.PB1, OutputType::PushPull));
            };
        ...
        if self.fan_0 || self.fan_1 {
            let mut fan_0_pwm_pin = None;
            if self.fan_0 {
                fan_0_pwm_pin = Some(PwmPin::new_ch2(p.PE11, OutputType::PushPull));
            };
            let mut fan_1_pwm_pin = None;
            if self.bed_heater {
                fan_1_pwm_pin = Some(PwmPin::new_ch1(p.PE9, OutputType::PushPull));
            };
            let pwm = SimplePwm::new(
                p.TIM1,
                fan_1_pwm_pin,
        ...
        if self.x_stepper || self.y_stepper || self.z_stepper || self.e_stepper {
            let usart = steppers::init_stepper_usart(p.USART2, p.PD5);
            if self.x_stepper {
                let stepper = steppers::init_x_stepper(usart, p.PD3, p.PD1, p.PD0, p.PE2, p.EXTI2);
                board.x_stepper = Some(stepper);
            }
            if self.y_stepper {
                let stepper =
                    steppers::init_y_stepper(usart, p.PD14, p.PD13, p.PD12, p.PE1, p.EXTI1);
                board.y_stepper = Some(stepper);
            }
            if self.z_stepper {
                let stepper = steppers::init_z_stepper(usart, p.PD2, p.PD4, p.PD15, p.PE5, p.EXTI5);
                board.z_stepper = Some(stepper);
            }
            if self.e_stepper {
                let stepper = steppers::init_e_stepper(usart, p.PD10, p.PD9, p.PD8, p.PA15);
                board.e_stepper = Some(stepper);
            }
        }

        if self.eeprom {
            let eeprom = eeprom::init_eeprom(p.I2C1, p.PB8, p.PB9, p.DMA1_CH6, p.DMA1_CH0);
            board.eeprom = Some(eeprom);
        }

        if self.flash {
            let flash = flash::init_flash(
        ...
        if self.display {
            let display = display::init_display(p.SPI2, p.PB10, p.PC3, p.PC2, p.PC9, p.PD11, p.PC8);
...
impl<'a> Board<'a> {
    pub async fn new(spawner: &'a Spawner, mac_addr: [u8; 6]) -> Self {
        ...
        // Init statics
        let adc = Self::init_adc1(p.ADC1);
        let usart = Self::init_stepper_usart(p.USART2, p.PD5);
        ...
        let hotend = Self::init_hotend_thermistor(adc, p.PC0, 4_092.0, 100_000.0, 25.0);
        let bed = Self::init_bed_thermistor(adc, p.PA4, 4_550.0, 100_000.0, 25.0);
        let board = Self::init_board_thermistor(adc, p.PA5, 4_267.0, 100_000.0, 25.0);
        let thermistors = BuddyThermistors { hotend, bed, board };

        // Bed Power Monitor (should not fail as we have initialised ADC1).
        let bed_power = Self::init_bed_power_monitor(p.PA3, adc);

        // Stepper motors (should not fail as we have initialised USART2).
        let x_stepper = Self::init_x_stepper(usart, p.PD3, p.PD1, p.PD0, p.PE2, p.EXTI2);
        let y_stepper = Self::init_y_stepper(usart, p.PD14, p.PD13, p.PD12, p.PE1, p.EXTI1);
        let z_stepper = Self::init_z_stepper(usart, p.PD2, p.PD4, p.PD15, p.PE5, p.EXTI5);
        let e_stepper = Self::init_e_stepper(usart, p.PD10, p.PD9, p.PD8, p.PA15);
    ...
    pub fn init_adc1(adc: ADC1) -> &'static BuddyAdc<ADC1> {
        BuddyAdc::new_static_adc1(adc)
    }
    ...
    pub fn init_bed_power_monitor(pin: PA3, adc: &'static BuddyAdc<ADC1>) -> BuddyBedPowerMonitor {
        bed_power_monitor::init_buddy_bed_power_monitor(adc, pin.degrade_adc())
    }

    pub fn init_stepper_usart(usart: USART2, tx: PD5) -> &'static BuddyUart {
    ...
    pub fn init_default_bed_thermistor(
        adc: &'static BuddyAdc<ADC1>,
        pin: PA4,
    ) -> BuddyThermistor<'a> {
        Self::init_bed_thermistor(adc, pin, 4_092.0, 100_000.0, 25.0)
    }
    ...
    pub fn init_bed_thermistor(
        ...
        Thermistor::new(adc, pin.degrade_adc(), 4_700.0, beta, r_ref, t_ref)
    }

    pub fn init_default_board_thermistor(
        adc: &'static BuddyAdc<ADC1>,
        pin: PA5,
    ) -> BuddyThermistor<'a> {
        Self::init_board_thermistor(adc, pin, 4_550.0, 100_000.0, 25.0)
    }
    ...
    pub fn init_default_hotend_thermistor(
        adc: &'static BuddyAdc<ADC1>,
        pin: PC0,
    ) -> BuddyThermistor<'a> {
        Self::init_hotend_thermistor(adc, pin, 4_267.0, 100_000.0, 25.0)
    }
    ...
    pub fn init_x_stepper(
        ...
        steppers::init_x_stepper(uart, en, step, dir, dia_pin, dia_ch)
    }
    ...
    pub fn init_y_stepper(
        ...
        steppers::init_y_stepper(uart, en, step, dir, dia_pin, dia_ch)
    }
    ...
    pub fn init_z_stepper(
        ...
        steppers::init_z_stepper(uart, en, step, dir, dia_pin, dia_ch)
    }
    ...
    pub fn init_e_stepper(
        ...
        steppers::init_e_stepper(uart, en, step, dir, dia_pin)
    }
